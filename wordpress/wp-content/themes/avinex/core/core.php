<?php
function avinex_child_css() {
	$avinex_parent_theme_css = 'flixita-style';
	wp_enqueue_style( 
		$avinex_parent_theme_css, 
		get_template_directory_uri() . '/style.css' 
	);
	wp_enqueue_style( 
		'avinex-style', 
		get_stylesheet_uri(), 
		array( $avinex_parent_theme_css )
	);
}
add_action( 'wp_enqueue_scripts', 'avinex_child_css',999);

require get_stylesheet_directory() . '/core/customizer/custom-controls/customize-upgrade-control.php';

function avinex_custom_header_setup()
{
...
add_action('after_setup_theme', 'avinex_custom_header_setup');

/**
 * Import Options From Parent Theme
 *
 */
function avinex_parent_theme_options() {
...
add_action( 'after_switch_theme', 'avinex_parent_theme_options' );